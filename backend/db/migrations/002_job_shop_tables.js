...
export async function up(client) {
  await client.query(workCentersTable);
  await client.query("ALTER TABLE work_centers ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()");
  await client.query(workOrdersTable);
  await client.query("ALTER TABLE work_orders ADD COLUMN IF NOT EXISTS part_number TEXT NOT NULL DEFAULT '', ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()");
  await client.query(schedulesTable);
}

export async function down(client) {
  await client.query("DROP TABLE IF EXISTS schedules");
  await client.query("DROP TABLE IF EXISTS work_orders");
  await client.query("DROP TABLE IF EXISTS work_centers");
}
